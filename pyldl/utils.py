...
def random_missing(y, missing_rate=.9):
    # unreasonable setting in IncomLDL
...
def random_missing_real(y, missing_rate=.9):
    # reasonable setting in HidLDL
    if missing_rate <= 0. or missing_rate >= 1.:
        raise ValueError("Invalid missing rate, which should be in the range (0, 1).")
    ...
    # 确保每一行至少有一个非零值
    for i in range(missing_y.shape[0]):
        if np.isnan(missing_y[i]).all():  # 如果一行中所有值都是NaN
            # print(f"第{i}行出现异常,填补中")
            random_index = np.random.choice(np.arange(missing_y.shape[1]))
            missing_y[i, random_index] = y[i, random_index]  # 将该值保留原值
            missing_mask[i, random_index] = False
    # 将NaN替换为0
    missing_y[np.isnan(missing_y)] = 0.
    sums = missing_y.sum(axis=1, keepdims=True)

    zero_sum_indices = np.where(sums == 0)[0]

    for index in zero_sum_indices:
        # print(index)
        # print(missing_y[index, :], y[index, :], missing_mask[index, :])
        # 获取当前行中mask为False的位置
        false_positions = np.where(missing_mask[index, :] == False)[0]
        # 如果存在mask为False的位置，则随机选择一个改为True
        if false_positions.size > 0:
            random_false_index = np.random.choice(false_positions)
            missing_mask[index, random_false_index] = True

        non_zero_positions = np.where(y[index, :] != 0)[0]
        # 如果存在array值不为0的位置，则随机选择一个mask置为False
        if non_zero_positions.size > 0:
            random_non_zero_index = np.random.choice(non_zero_positions)
            missing_mask[index, random_non_zero_index] = False
            missing_y[index, random_non_zero_index] = y[index, random_non_zero_index]
        # print(missing_y[index, :], y[index, :], missing_mask[index, :])
        # print("next")

    sums = missing_y.sum(axis=1, keepdims=True)
    if np.any(sums == 0):
        print("错误,调整后还出现了全是0的行")
    missing_y = missing_y / sums
    return missing_y, missing_mask
