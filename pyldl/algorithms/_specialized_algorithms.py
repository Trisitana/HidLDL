...
from pyldl.algorithms import LDSVR,AA_KNN
...
class SA_BFGS(_SA):
    ...
    def predict1(self, X):
        bfgs = SA_BFGS()
        bfgs.fit(self._X, self.predict(self._X))
        return bfgs.predict(X)
...
class SA_IIS(_SA):
    ...
    def fit(self, X, y, max_iterations=150, convergence_criterion=1e-6):
        super().fit(X, y)
